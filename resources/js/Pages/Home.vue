...
<template>
  <div class="min-h-screen bg-[#f9f9ff] text-[#111c2d] antialiased overflow-x-hidden w-full max-w-full">
    ...
    <main class="w-full max-w-full overflow-x-hidden">
      ...
      <section class="relative w-full max-w-full overflow-hidden bg-[#06182A] text-white">
        <div class="relative w-full max-w-full h-[540px] sm:h-[580px] lg:h-[650px] overflow-hidden">
          <div v-for="(slide, index) in heroSlides" :key="slide.id"
            ...
            <div class="relative max-w-7xl mx-auto h-full px-4 sm:px-6 flex flex-col justify-center">
              <div class="max-w-2xl space-y-4">
                <span :class="['inline-flex items-center gap-1.5 text-[10px] sm:text-xs font-bold px-3 py-1 rounded-full shadow-md tracking-wider uppercase', slide.tagColor]">
                  <span class="material-symbols-outlined text-[14px]">local_fire_department</span>
                  {{ slide.tag }}
                </span>
                <h1 class="text-2xl sm:text-4xl lg:text-5xl font-extrabold leading-tight tracking-tight text-white break-words">
                  {{ slide.title }}
                </h1>
                <p class="text-slate-300 text-xs sm:text-sm lg:text-base leading-relaxed line-clamp-3 sm:line-clamp-none">
                  {{ slide.description }}
                </p>
                <div class="pt-2 flex flex-wrap items-center gap-3 sm:gap-4">
                  ...
                  <a href="#lineup-section" class="h-11 sm:h-12 px-5 sm:px-6 bg-white/15 hover:bg-white/25 text-white font-bold text-xs sm:text-sm rounded-xl backdrop-blur-sm flex items-center gap-2 transition-all">
                    {{ slide.btnSecondary }}
          ...
          <!-- Slider Controls -->
          <div class="absolute bottom-16 sm:bottom-20 left-0 right-0 z-20">
            <div class="max-w-7xl mx-auto px-4 sm:px-6 flex items-center justify-between">
              <div class="flex items-center gap-2 sm:gap-3">
                <button v-for="(_, idx) in heroSlides" :key="idx" @click="currentSlide = idx"
                        :class="['h-2 sm:h-2.5 transition-all rounded-full cursor-pointer', currentSlide === idx ? 'w-8 sm:w-10 bg-red-600' : 'w-2.5 sm:w-3 bg-white/40']"></button>
              </div>
              <div class="flex items-center gap-2">
                <div class="bg-[#06182A]/80 backdrop-blur-md px-2.5 py-1 rounded-full text-[10px] sm:text-xs font-mono text-slate-300">
                  <strong class="text-white">0{{ currentSlide + 1 }}</strong> / 0{{ heroSlides.length }}
                </div>
                <button @click="prevSlide" class="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-white/20 hover:bg-white text-white hover:text-slate-900 transition-all flex items-center justify-center cursor-pointer">
                  <span class="material-symbols-outlined text-[18px] sm:text-[20px]">chevron_left</span>
                </button>
                <button @click="nextSlide" class="w-8 h-8 sm:w-9 sm:h-9 rounded-full bg-white/20 hover:bg-white text-white hover:text-slate-900 transition-all flex items-center justify-center cursor-pointer">
      ...
      <section class="relative z-30 -mt-8 sm:-mt-12 max-w-7xl mx-auto px-4 sm:px-6 w-full box-border">
        <div class="grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
          ...
          <a href="https://wa.me/6285299837635?text=Halo%20CHAE%20SUZUKI,%20saya%20mau%20konsultasi%20tukar%20tambah%20mobil%20bekas" target="_blank" class="group bg-white rounded-xl p-5 sm:p-6 shadow-md hover:shadow-xl transition-all border border-slate-200 flex flex-col justify-between">
            <div class="space-y-3">
            ...
            <div class="pt-4 sm:pt-5 flex items-center gap-1.5 text-xs font-bold text-emerald-600">
      ...
      <section class="w-full py-12 sm:py-16" id="lineup-section">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 space-y-8">
          <div class="flex flex-col md:flex-row md:items-end justify-between gap-4">
            <div>
              ...
              <p class="text-xs sm:text-sm text-slate-500">Pilihan unit ready stock dengan penawaran bunga kompetitif, DP minim promo terbaru, dan proses leasing cepat.</p>
            </div>

            <!-- Filter Tabs -->
            <div class="flex flex-wrap items-center gap-1 bg-slate-100 p-1 rounded-xl self-start md:self-auto max-w-full">
              <button @click="selectedCategory = 'all'" :class="['px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-bold rounded-lg transition-all cursor-pointer', selectedCategory === 'all' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900']">Semua Model</button>
              <button @click="selectedCategory = 'passenger'" :class="['px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-bold rounded-lg transition-all cursor-pointer', selectedCategory === 'passenger' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900']">Passenger Car</button>
              <button @click="selectedCategory = 'commercial'" :class="['px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-bold rounded-lg transition-all cursor-pointer', selectedCategory === 'commercial' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900']">Commercial Niaga</button>
              <button @click="selectedCategory = 'hybrid'" :class="['px-3 sm:px-4 py-1.5 sm:py-2 text-xs font-bold rounded-lg transition-all cursor-pointer', selectedCategory === 'hybrid' ? 'bg-white text-slate-900 shadow-sm' : 'text-slate-600 hover:text-slate-900']">Smart Hybrid</button>
            </div>
          </div>
      ...
      <section class="w-full py-12 sm:py-16">
        <div class="max-w-7xl mx-auto px-4 sm:px-6 space-y-10">
          <div class="text-center max-w-2xl mx-auto space-y-1">
            ...
            <p class="text-xs sm:text-sm text-slate-500">Memberikan kemudahan kepemilikan mobil baru Suzuki dengan pelayanan terstandarisasi PT Megahputra Sejahtera Makassar.</p>
          </div>

          <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 sm:gap-6">
            <div class="bg-white p-5 sm:p-6 rounded-xl border border-slate-200 shadow-sm space-y-3">
          ...
          <div class="relative rounded-2xl overflow-hidden bg-[#06182A] text-white p-6 sm:p-8 lg:p-12 shadow-2xl">
            <div class="relative z-10 grid grid-cols-1 lg:grid-cols-12 gap-8 items-center">
              <div class="lg:col-span-8 space-y-4">
                <div class="inline-flex items-center gap-2 bg-slate-800 px-3 py-1 rounded-full text-[11px] sm:text-xs font-bold text-slate-300">
              ...
              <div class="lg:col-span-4 flex justify-center lg:justify-end">
                <div class="bg-white/10 backdrop-blur-md p-6 rounded-2xl border border-slate-700 text-center space-y-3 max-w-xs w-full">
                  ...
                  <div>
                    <h3 class="font-bold text-white text-base sm:text-lg">CHAE SUZUKI</h3>
                    <p class="text-[11px] sm:text-xs text-red-400 font-bold uppercase tracking-wider">Sales Executive</p>
                    <p class="text-[11px] sm:text-xs text-slate-400">PT Megahputra Sejahtera (3S)</p>
                  </div>
                  <p class="text-[10px] sm:text-[11px] text-slate-300 italic">"Komitmen melayani Bapak/Ibu dengan ramah, cepat, jujur, dan transparan di seluruh Sulawesi Selatan."</p>
                </div>
              </div>
            </div>
          </div>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8 pb-12 border-b border-slate-800">
          <div class="space-y-4">
            <div class="flex items-center gap-3">
              ...
              <span class="font-bold text-base sm:text-lg uppercase tracking-tight text-white">Megahputra Sejahtera Makassar</span>
            </div>
